...
void anime_perspective(inout float4 v) {
    ...
    float3 viewdir = WorldSpaceViewDir(v); // Vector from vertex to camera
    ...
    v.xyz += mul(unity_WorldToObject, (-cameradir) * len * factor + (viewdir - cameradir) * len * factor); // Move vertices
}
...
//    float3 worldView = normalize(lerp(_WorldSpaceCameraPos.xyz - i.posWorld.xyz, UNITY_MATRIX_V[2].xyz, unity_OrthoParams.w));
    float3 worldView = unity_OrthoParams.w;
...
#endif
    float3 worldViewRim = normalize(lerp(_WorldSpaceCameraPos.xyz - i.posWorld.xyz, UNITY_MATRIX_V[2].xyz, unity_OrthoParams.w));
    half3 rimLighting = lerp(staticRimLighting, mixedRimLighting, _RimLightingMix);
    half3 rim = pow(saturate(1.0 - dot(worldNormal, worldViewRim) + _RimLift), _RimFresnelPower) * _RimColor.rgb * tex2D(_RimTexture, mainUv).rgb;
    ...
    half3 worldViewUp = normalize(worldCameraUp - worldViewRim * dot(worldViewRim, worldCameraUp));
    half3 worldViewRight = normalize(cross(worldViewRim, worldViewUp));
